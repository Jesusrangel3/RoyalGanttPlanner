...
sql.connect(config).then(pool => {
  return pool.request().query("SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Tasks'");
}).then(res => {
  console.log("COLUMNAS DE TASKS EN DB:", JSON.stringify(res.recordset, null, 2));
  process.exit(0);
}).catch(err => {
  console.error(err);
  process.exit(1);
