...
export async function GET(request: Request) {
  ...
  try {
    const query = taskId
      ? `SELECT TOP 50 * FROM ApprovalLog_Gantt WHERE taskId = @taskId ORDER BY createdAt DESC`
      : `SELECT TOP 100 * FROM ApprovalLog_Gantt ORDER BY createdAt DESC`;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await executeQuery(`
      INSERT INTO ApprovalLog_Gantt (id, taskId, taskTitle, action, performedBy, performedByName, comment, previousStatus)
